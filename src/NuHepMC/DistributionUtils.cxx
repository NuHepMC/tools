...
/// Template class to act as a helper for generating ranges in linear space.
/// This is done in a way to mimic the linspace function found within the numpy
/// python library.
...
public:
  /// Construct a helper object for generating equally spaced points in linear
  /// space
...
/// Function to generate a range of numbers equally spaced in linear space from
/// start to stop. This function mimics the linspace function found within the
...
///@return std::vector<double>: A vector containing equally spaced points in
///linear space
inline std::vector<double> Linspace(double start, double stop,
  ...
  std::generate_n(std::back_inserter(retval), num,
                  LinspaceGen<double>(start, step));
  return retval;
}

Histogram::Histogram(std::string const &name, size_t nb, double min, double max)
    : hist_name(name), nbins(nb), bin_edges(Linspace(min, max, nb + 1)),
      heights(nb), errors(nb) {}

Histogram::Histogram(std::string const &name, std::vector<double> const &edges)
    : hist_name(name), nbins(edges.size()), bin_edges(edges),
      heights(edges.size() - 1), errors(edges.size() - 1) {}

void Histogram::Fill(double x, double wgt) {
  // Underflow and Overflow check
  if (x < bin_edges.front() || x > bin_edges.back())
    return;
  ...
  *out << fmt::format("{:>15},{:>15},{:>15},{:>15}\n", "lower edge",
  ...
  for (size_t i = 0; i < heights.size(); ++i) {
    *out << fmt::format("{:> 15.6e},{:> 15.6e},{:> 15.6e},{:> 15.6e}\n",
                        bin_edges[i], bin_edges[i + 1], heights[i],
                        std::sqrt(errors[i]));
  }
}
...
void PlotFinalStateLeptonEnergy(std::string const &Filename) {
  ...
  std::cout << "xsec: " << xsec << ", weight_sum = " << weight_sum
            << ": SF = " << (xsec / weight_sum) << std::endl;
